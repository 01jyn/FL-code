import sys
sys.path.append(r'/home/jxj/Robust_FL-Master-cf/Robust_FL-master/')
print(sys.path)
#sys.path.append("..")
...
TestBatchSize = 512 #512
CommunicationEpoch = 200
Pariticpant_Params = {
    'loss_funnction' : 'SCE',
    'optimizer_name' : 'Adam',
    'learning_rate'  : 0.001
...
def evaluate_network(network,dataloader,logger):
    ...
    return acc
def get_min_loss(images_list, net, criterion):
    res = images_list[0]
    min_loss = 1000
    for image, lable in images_list:
        per,_ = net(image)
        loss = criterion(per,lable)
